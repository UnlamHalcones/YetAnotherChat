...
public class ServidorChat {
	...
	/*
	 * public void disconnect() { cortarConexionesDeUsuarios(); try {
	 * serverSocket.close(); } catch (IOException e) { e.printStackTrace(); } }
	 * 
	 * private void cortarConexionesDeUsuarios() {
	 * this.userThreads.forEach(threadUsuario -> {
	 * threadUsuario.sendMessage("DISCONNECT"); }); this.userThreads = new
	 * HashSet<>(); }
	 */
	...
	public Command crearSala(String nombreSala, Usuario usuario) {
		...
		Optional<SalaChat> optSalaConMismoNombre = this.salasInServer.stream()
				.filter(salaChat -> salaChat.getNombreSala().equalsIgnoreCase(nombreSala)).findAny();
		if (!optSalaConMismoNombre.isPresent()) {
			SalaChat nuevaSalaChat = new SalaChat(
					salasInServer.isEmpty() ? 1L : salasInServer.get(salasInServer.size() - 1).getId() + 1L, nombreSala,
					usuario);
	...
	private SalaChat existeSalaConId(Long id) {
		Optional<SalaChat> salaChat = this.salasInServer.stream().filter(sala -> sala.getId().equals(id)).findAny();
